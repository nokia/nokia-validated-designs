...
def _build_group_vars_all(intent: FabricIntent) -> dict[str, Any]:
    ...
    if intent.environment == PHYSICAL:
        gv["ansible_password_vault_recommended"] = True
    return gv
...
def _readme(fabric_name: str, generated_at: str = "", environment: str = "containerlab") -> str:
    ...
    if environment == PHYSICAL:
        env_note = "\n\n> **Target:** physical hardware (`environment: physical`). Move `ansible_password` from `group_vars/all.yml` into Ansible Vault before production use."
    body = textwrap.dedent("""\
        # {fabric_name} — Ansible Project

        Auto-generated by the NVD Ansible generator.{staleness_note}{env_note}
...
def generate(intent: FabricIntent, output_dir: Path | str | None = None) -> Path:
    ...
    _write_text(output_dir / "README.md", _readme(intent.fabric_name, generated_at, intent.environment))
