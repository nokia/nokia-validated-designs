...
class InterfaceMultiHoming(_EDABase):
    df_election: Literal['Default', 'HighestPreference'] | None = Field(None, alias="dfElection", description="Selects the DF election algorithm. If not specified, `default` is used with `AllActive` multi-homing and `HighestPref...", title="DF Election Algorithm")
    ...
    mode: Literal['AllActive', 'SingleActive', 'PortActive'] | None = Field('AllActive', description="\"AllActive\": All interfaces are active. \"SingleActive\": In a single active MH LAG, the active and standby functio...", title="Mode")
    preferred_active_node: str | None = Field(None, alias="preferredActiveNode", description="When configured with a reference to the Node object and the Highest-Preference DF election algorightm is used, the se...", title="Preferred Active Node")
    reload_delay_timer_seconds: int | None = Field(100, alias="reloadDelayTimerSeconds", description="After the system boots, the reload-delay timer in seconds keeps an interface shut down with the laser off for a confi...", title="Reload Delay Timer", ge=1, le=86400)
    revertive: bool | None = Field(False, description="Enables revertive DF election behavior with Preference-Based Election mode. [default=false]", title="Revertive")
...
class InterfaceFallback(_EDABase):
    ...
    timeout_seconds: int | None = Field(60, alias="timeoutSeconds", description="Specifies the LACP-fallback timeout interval in seconds. [default=60]", title="Timeout", ge=4, le=3600)
...
class InterfaceLacp(_EDABase):
    ...
    system_mac: str | None = Field(None, alias="systemMAC", description="The MAC address portion of the Node's System ID. This is combined with the system priority to construct the 8-octet s...", title="System ID MAC")
...
class InterfaceLag(_EDABase):
    lacp: InterfaceLacp | None = Field(None, description="LACP properties. This section is only applicable if the LAG type is set to LACP.", title="LACP")
    ...
    multihoming: InterfaceMultiHoming | None = Field(None, description="Multi-homing properties.", title="Multi Homing")
...
class InterfaceStormControl(_EDABase):
    ...
    unit: Literal['Kbps', 'BandwidthPercentage'] | None = Field(None, description="Set the unit of measurement for the storm control rates.", title="Unit")
...
class InterfaceCrcMonitor(_EDABase):
    ...
    window_size_seconds: int | None = Field(None, alias="windowSizeSeconds", description="Sliding window size over which CRC errors are measured, in number of seconds.", title="Window Size", ge=1)
...
class InterfaceEthernet(_EDABase):
    ...
    hold_down_timer_ms: int | None = Field(None, alias="holdDownTimerMs", description="The hold-time down behavior is triggered with events that try to bring the ethernet interface down and can change qui...", title="Hold Down Timer", ge=100, le=86400000)
    hold_up_timer_ms: int | None = Field(None, alias="holdUpTimerMs", description="The hold-time up behavior is triggered with any event that tries to bring up the ethernet interface.  While the hold-...", title="Hold Up Timer", ge=100, le=86400000)
    ...
    reload_delay_timer_seconds: int | None = Field(None, alias="reloadDelayTimerSeconds", description="After the system boots, the reload-delay timer in seconds keeps an interface shut down with the laser off for a confi...", title="Reload Delay Timer", ge=1, le=86400)
    ...
    standby_signaling: Literal['LACP', 'PowerOff'] | None = Field(None, alias="standbySignaling", description="Indicates the standby-signaling used in the interface.", title="Standby Signaling")
...
class InterfaceSpec(_EDABase):
    ...
    mode: Literal['Access', 'Network', 'Hybrid'] | None = Field(None, description="Set interface mode for platforms that support explicit configuration of access or network mode. [SROS only]", title="Mode")
    mtu: int | None = Field(None, description="MTU to apply on the interface(s).", title="MTU", ge=1450, le=9500)
    type: Literal['LAG', 'Interface', 'Loopback'] | None = Field('Interface', description="Type defines whether the interface is a LAG or a regular Interface.", title="Type")
