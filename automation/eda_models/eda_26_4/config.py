...
class ConfigletConfigurations(_EDABase):
    config: str | None = Field(None, description="JSON-formatted string representing the configuration to apply.", title="Configuration")
...
class ConfigletSpec(_EDABase):
    ...
    endpoint_selectors: list[str] | None = Field(None, alias="endpointSelectors", description="Label selector to use to match targets to deploy Configlet to.", title="Target Selector")
    endpoints: list[str] | None = Field(None, description="Reference to targets to deploy Configlet to.", title="Targets")
    operating_system: Literal['srl', 'sros', 'eos', 'sonic', 'ios-xr', 'nxos'] | None = Field(None, alias="operatingSystem", description="Operating system to match against when selecting targets.", title="Operating System")
