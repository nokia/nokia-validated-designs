...
**5. `advertise-ifl-host-ad-routes` — Configlet.** No field for it on `Interface`, `Router` or `RouterInterconnect`, and setting it by hand does not survive: it sits inside the subtree EDA owns for the ethernet segment, so the next reconcile removes it. Without it a multi-homed host's /32 is advertised only by whichever leaf ARPed it, and the whole fabric pins routed traffic for that host to that one leaf — all-active for bridged traffic, effectively single-active for routed.
