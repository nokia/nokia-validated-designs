...
This AI NVD digital twin deployment is a two-stripe rail-optimized design with eight leafs per stripe and two spines acting as inter-stripe connectors. The topology, shown below, is deployed/instantiated using Containerlab and orchestrated by EDA.

![two-stripe-rail-optimized](/static/images/two-stripe-rail-optimized.png)

This AI NVD covers the following features:

- IPv6 fabric connectivity
...
Once you have confirmed that the EDA license is installed and valid (using `kubectl describe license -A`) and the fabric nodes are reachable via the bootstrap server pod `bsvr`, you can then run the deploy script with `bash ./deploy-ai-fabric.sh`. This is a prescriptive bash script built specifically for the Containerlab NVD topology found in this repository. It deploys all the necessary EDA components to bring the fabric to life (the script simply does a `kubectl apply -f [manifest-file]` for the individual manifest files and ensures that they are successfully applied before proceeding to the next file). For this NVD, the deploy script also uses an Ansible playbook to configure the Ubuntu servers (acting as GPU servers, in this case) with their IPv6 addresses - you must ensure Ansible is installed in your environment to execute this.

### Destroying the fabric

For quick teardown, another bash script called `destroy-ai-fabric.sh` is provided as well. Again, this is a prescriptive script, specific to what is deployed using the `deploy-ai-fabric.sh` script.

### Revision history

| EDA Version  | SRL version | Date tested |
|--------------|-------------|-------------|
| 25.8.3 | 25.10.1 | December 2025 |
